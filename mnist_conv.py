...
class MnistConv():
    ...
    def build_graph(self):
        ...
        out = self.x
        out = tf.layers.conv2d(out,32,3,strides=(2,2),activation=tf.nn.relu,padding='same',kernel_initializer=xinit(),use_bias=False)
        out = tf.layers.conv2d(out,64,3,strides=(2,2),activation=tf.nn.relu,padding='same',kernel_initializer=xinit(),use_bias=False)
        out = tf.layers.conv2d(out,128,3,strides=(2,2),activation=tf.nn.relu,padding='same',kernel_initializer=xinit(),use_bias=False)
